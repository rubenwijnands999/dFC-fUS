...
class GaussianObservations(Observations):
    ...
    def m_step(self, expectations, datas, inputs, masks, tags, **kwargs):
        ...
        J = np.zeros((K, D))
        h = np.zeros((K, D))
        for (Ez, _, _), y in zip(expectations, datas): #expectations consist of expected_states, expected_joints, normalizer
...
#             for _ in range(K)])
#
#         #dimension of K, concatenated matrix {A-1,A-2...}
#
...
#         D_in = D * lags + M + 1  #1 for intercept?
#
...
#                 Ens[k] += np.sum(w)  # N_k, this was me prob
#
